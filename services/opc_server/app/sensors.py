...
class VoltageSensor(object):
    ...
    def __init__(self,  base_url: str):
        self.name = "voltageSensor"
        self.base_url = base_url
        self.sources = [
            {
                "name": "ADS1115 ADC Sensor",
                "datatype": ua.Variant(0.0, ua.VariantType.Double),
                "historize": True,
                "historize_length": 1000,
                "func": self.read_value,
    ...
    def read_value(self):
        """
        Fetches the voltage `/voltage` endpoint.

        Args:
            base_url (str): The base URL of the API including the endpoint path. Defaults to `http://localhost:8000/voltage`.

        Returns:
            float: The voltage read from thermal-electrical generator.
        ...
        try:
            ...
            # Try to parse the response as a temperature value
            voltage = float(response.text.replace('"',''))  # Assuming the response is a number in string format
            return voltage
