...
def _healthcheck_temp_sensors():
    ...
    try:
        ...
        if (not math.isnan(temp_upper) and -20 <= temp_upper <= 125 and
            not math.isnan(temp_lower) and -20 <= temp_lower <= 125):
            return {
                "temperature_upper": temp_upper,
                "temperature_lower": temp_lower,
                "status": "OK"
            ...
            logger.warning(f"Temperature sensor readings out of range: upper={temp_upper}, lower={temp_lower}")
            return False
    except Exception as e:
...
def _healthcheck_audio_sensor():
    ...
    try:
        ...
        if (mfcc_data and spectrum_data and 
            isinstance(mfcc_data, dict) and isinstance(spectrum_data, dict) and
            len(mfcc_data) > 0 and len(spectrum_data) > 0):
            return {
                "audio_processing": "OK",
...
def _healthcheck_settings_integrity():
    ...
    try:
        ...
        rpm_valid = (
            rpm_settings["measurement_window"] > 0 and
            rpm_settings["measurement_interval"] > 0 and
            rpm_settings["sample_size"] > 0
        )
        
        # Check audio settings
        audio_valid = (
            audio_settings["sample_duration"] > 0 and
            audio_settings["mfcc_count"] > 0 and
            audio_settings["buffer_size"] > 0
        )
