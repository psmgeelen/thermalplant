...
class AudioHandler:
    ...
    def _find_audio_device(self,
                           match_on: str = "alsa_input.usb-GeneralPlus_USB_Audio_Device-00.mono-fallback") -> list:
        matches = []
        try:
            ...
            try:
                ...
                if self.processing_loop.is_ready():
                    ...
                    if time_since_last_sync > 10.0 and not recovery_in_progress:  # 10 seconds without data
                        logger.warning(
                            f"No audio data processed for {time_since_last_sync:.1f} seconds, attempting recovery")
                        self._recover_audio_components()
